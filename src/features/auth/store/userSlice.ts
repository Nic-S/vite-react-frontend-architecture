...
export const loginAndFetchUser = (): AppThunk => async (dispatch, state) => {
...
export const fetchUser = (): AppThunk => async (dispatch, state) => {
  try {
...
export const refreshTokenAndFetchUser = (): AppThunk => async (dispatch, state) => {
  if (!state().user.isFetching) {
    dispatch(startFetch());
    try {
